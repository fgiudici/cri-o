...
[![GoDoc](https://godoc.org/google.golang.org/old_grpc?status.svg)](https://godoc.org/google.golang.org/old_grpc)
...
```
$ go get -u google.golang.org/old_grpc
```

With Go module support (Go 1.11+), simply `import "google.golang.org/old_grpc"` in
...
Documentation
-------------
- See [godoc](https://godoc.org/google.golang.org/old_grpc) for package and API
  descriptions.
...
```
$ go get -u google.golang.org/old_grpc
package google.golang.org/old_grpc: unrecognized import path "google.golang.org/old_grpc" (https fetch: Get https://google.golang.org/old_grpc?go-get=1: dial tcp 216.239.37.1:443: i/o timeout)
```
...
- Without Go module support: `git clone` the repo manually:
  ...
  ```
  git clone https://github.com/grpc/grpc-go.git $GOPATH/src/google.golang.org/old_grpc
  ```
...
- With Go module support: it is possible to use the `replace` feature of `go
  ...
  ```
  go mod edit -replace=google.golang.org/old_grpc=github.com/grpc/grpc-go@latest
  go mod tidy
  go mod vendor
  go build -mod=vendor
  ```
...
require (
    google.golang.org/old_grpc v1.27.0
)
```
...
possible reasons, including:
 ...
 1. Keepalive parameters caused connection shutdown, for example if you have configured
    ...
    If this is the case, you may want to increase your [MaxConnectionAgeGrace](https://pkg.go.dev/google.golang.org/old_grpc/keepalive?tab=doc#ServerParameters),
